...
const guidebook = fs.readFileSync(path.join(root, "guidebook.html"), "utf8");
const plainText = html.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ");
const guidebookText = guidebook.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ");
const required = [
...
const guidebookRequired = [
  "웹사이트를 직접 공부하는 Codex 자습 가이드북",
  "혼자 따라 하는 실습 미션",
  "복사해서 쓰는 요청문 모음",
  "Vercel 배포",
];

for (const text of guidebookRequired) {
  if (!guidebook.includes(text) && !guidebookText.includes(text)) {
    throw new Error(`Missing guidebook text: ${text}`);
  }
}
